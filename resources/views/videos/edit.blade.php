        ...
        <form method="post" action="{{ Route('video.update', $video->id) }}" enctype="multipart/form-data"
            ...
            <div class="w-full flex items-center justify-center mb-6">
                <label for="dropzone-file"
                    ...
                    @if ($video->video)
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400"><span class="font-semibold">Uploaded
                                    File:</span> {{ $video->video }}</p>
                        </div>
                    @else
            ...
            <x-text-input name="name" class="mb-4" placeholder="Title..." :value="$video->name">
            </x-text-input>
